...
def configure_arg_parser():
    ...
    parser.add_argument("-cl", "--max_context", type=int, default=128, help="Max length of context fed into diffusion")
    ...
    parser.add_argument("-df", "--diffusion_steps", type=int, default=100, help="Number of diffusion steps")
    parser.add_argument("-s", "--num_steps", type=int, default=1000, help="Number of training epochs")

    return parser


def main(model, train, val, batch_size, side, max_context, max_gen, device, schedule, diffusion_steps, num_steps):
    train_dataset = ConvAI2Dataset(train)
    ...
    train_model(
        model, train_dataloader, val_dataloader, schedule, diffusion_steps, num_steps, max_context, max_gen, device
    )
