...
def configure_schedule(steps: int, schedule: str):
    ...
    sigma_0 = torch.sqrt(1 - alphas_cumprod_prev[0]).item()
...
def get_xt(x_0, alphas_cumprod_prev, t, noise=None):
    ...
    sigma_t = torch.sqrt(1 - alphas_cumprod_prev_t)
    x_t = torch.sqrt(alphas_cumprod_prev_t) * x_0 + sigma_t * noise
    return x_t, sigma_t
...
def get_diffusion_variables(diffusion_steps: int, emb: torch.Tensor, alphas_cumprod_prev: torch.Tensor, sigma_0: float):
    ...
    x_t, sigma_t = get_xt(x_0, alphas_cumprod_prev, t)
    return x_0, x_t, sigma_t, t
